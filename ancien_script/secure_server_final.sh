...
ecrire_question(){
	...
	do
		...
		elif [ -n $2 ] && [ $2 = "texte" ];then # accepte toutes les r�ponses non vides
			if [ ! -z $reponse ];then
				repok="ok" 
			fi
		elif [ -n $2 ] && [ $2 = "chiffre" ];then # accepte toutes les r�ponses en chiffre
			#printf -v int '%d\n' "$reponse" 2>/dev/null
			if [ ! -z $reponse ] && [[ "$reponse" =~ ^[0-9]+$ ]];then
				repok="ok" 
			fi
		else # accepte toutes les r�ponses meme vides
			repok="ok" 
		fi
...
if [ $help = "yes" ];then
	...
	if [ $protectscan = "o" ];then
		...
		do
			...
			#D�commentez la ligne KILL_ROUTE="/sbin/iptables -I INPUT-s $TARGET$ -j DROP"
	...
	if [ $protectrootkit="o" ];then
		...
		read -p "Veuillez entrer un mail pour re�evoir des rapports :" reponse
	...
	if [ $logwatch = "o" ];then
		...
		&& read -p "Veuillez entrer un mail pour re�evoir des rapports d'analyse de logs:" reponse \
